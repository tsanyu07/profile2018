...
<?php
    ...
    //頁面列表
    $sqlstr = "SELECT * FROM `".DBTable_Technology."` WHERE `del` != '1' AND `showup_switch` = '1' AND `name_jp` != '' ORDER BY `sort` ASC, `id` DESC";
    ...
    $sqlstr = "SELECT * FROM `".DBTable_Technology."` WHERE `del` != '1' AND `showup_switch` = '1' AND `name_jp` != '' AND `id` = '".$ID."'";
...
<?php include_once('_header.php'); ?>
	...
	<div class="main">
        <div class="bgbanner bgeducation">
            <div class="block">
            <h2>Technology <small>テクノロジー</small></h2>
            </div>
        </div>
...
<?php
    ...
    for ($i = 0; $i < sizeof($TechnologyList); $i++)
        echo "                	<li class=\"sidelist".($ID == $TechnologyList[$i]['id'] ? " selected" : "")."\"><i class=\"fa fa-caret-right\"></i><a href=\"technology.php?id=".$TechnologyList[$i]['id']."\">".$TechnologyList[$i]['name_jp']."</a></li>";

?>
                </ul>
                <div class="content">
                	<h2><?=$TechnologyData['name_jp']?></h2>                		
    <div class="edit">
      <?=$TechnologyData['contents_jp']?>
    </div>
        </div>
