...
<?php
    ...
    //商品列表
    $sqlstr = "SELECT * FROM `".DBTable_Product."` WHERE `del` != '1' AND `showup_switch` = '1' AND `name_tw` != '' AND `id` = '".$ID."'";
    $ProductData = $db->getRow($sqlstr);

    //下載列表
    $sqlstr = "SELECT * FROM `".DBTable_Product_Download."` WHERE `del` != '1' AND `showup_switch` = '1' AND `product_id` = '".$ID."' AND `type` = '1' AND `name_tw` != '' AND `file_tw` != '' ORDER BY `sort` ASC, `id` ASC";
    $DownloadType1List = $db->getAll($sqlstr);

    $sqlstr = "SELECT * FROM `".DBTable_Product_Download."` WHERE `del` != '1' AND `showup_switch` = '1' AND `product_id` = '".$ID."' AND `type` = '2' AND `name_tw` != '' AND `file_tw` != '' ORDER BY `sort` ASC, `id` ASC";
    $DownloadType2List = $db->getAll($sqlstr);

    $sqlstr = "SELECT * FROM `".DBTable_Product_Download."` WHERE `del` != '1' AND `showup_switch` = '1' AND `product_id` = '".$ID."' AND `type` = '3' AND `name_tw` != '' AND `file_tw` != '' ORDER BY `sort` ASC, `id` ASC";
...
?>
                		    ...
                		    <p class="info_txt"><?=nl2br($ProductData['introduction_tw'])?></p>
                		</div>
  <div id="tabs">
  <ul>
    <li><a href="#tabs-1">專家認證</a></li>
    <li><a href="#tabs-2">特色</a></li>
    <li><a href="#tabs-3">規格</a></li>
    <li><a href="#tabs-4">配件</a></li>
    <li><a href="#tabs-5">檔案下載</a></li>
  </ul>
  <div id="tabs-1" class="edit">
    <?=$ProductData['certification_tw']?>
  </div>
  <div id="tabs-2" class="edit">
    <?=$ProductData['feature_tw']?>
  </div>
  <div id="tabs-3" class="edit">
    <?=$ProductData['specification_tw']?>
  </div>
  <div id="tabs-4" class="edit">
    <?=$ProductData['accessory_tw']?>
  </div>
...
<?php
    ...
    if (sizeof($DownloadType1List) > 0) {
        ...
        echo "      <li>".$DownloadType[1]['tw']."：";

        for ($i = 0; $i < sizeof($DownloadType1List); $i++)
            if ($DownloadType1List[$i]['file_tw'] != "" && file_exists(Path_Upload.$DownloadType1List[$i]['file_tw']))
                echo "<a href=\"".Url_Upload.$DownloadType1List[$i]['file_tw']."\" target=\"_blank\">".$DownloadType1List[$i]['name_tw']."</a>";
    ...
    if (sizeof($DownloadType2List) > 0) {
        ...
        echo "      <li>".$DownloadType[2]['tw']."：";

        for ($i = 0; $i < sizeof($DownloadType2List); $i++)
            if ($DownloadType2List[$i]['file_tw'] != "" && file_exists(Path_Upload.$DownloadType2List[$i]['file_tw']))
                echo "<a href=\"".Url_Upload.$DownloadType2List[$i]['file_tw']."\" target=\"_blank\">".$DownloadType2List[$i]['name_tw']."</a>";
    ...
    if (sizeof($DownloadType3List) > 0) {
        ...
        echo "      <li>".$DownloadType[3]['tw']."：";

        for ($i = 0; $i < sizeof($DownloadType3List); $i++)
            if ($DownloadType3List[$i]['file_tw'] != "" && file_exists(Path_Upload.$DownloadType3List[$i]['file_tw']))
                echo "<a href=\"".Url_Upload.$DownloadType3List[$i]['file_tw']."\" target=\"_blank\">".$DownloadType3List[$i]['name_tw']."</a>";
