...
<?php
    ...
    //商品列表
    $sqlstr = "SELECT * FROM `".DBTable_Product."` WHERE `del` != '1' AND `showup_switch` = '1' AND `name` != '' AND `id` = '".$ID."'";
    $ProductData = $db->getRow($sqlstr);

    //下載列表
    $sqlstr = "SELECT * FROM `".DBTable_Product_Download."` WHERE `del` != '1' AND `showup_switch` = '1' AND `product_id` = '".$ID."' AND `type` = '1' AND `name` != '' AND `file` != '' ORDER BY `sort` ASC, `id` ASC";
    $DownloadType1List = $db->getAll($sqlstr);

    $sqlstr = "SELECT * FROM `".DBTable_Product_Download."` WHERE `del` != '1' AND `showup_switch` = '1' AND `product_id` = '".$ID."' AND `type` = '2' AND `name` != '' AND `file` != '' ORDER BY `sort` ASC, `id` ASC";
    $DownloadType2List = $db->getAll($sqlstr);

    $sqlstr = "SELECT * FROM `".DBTable_Product_Download."` WHERE `del` != '1' AND `showup_switch` = '1' AND `product_id` = '".$ID."' AND `type` = '3' AND `name` != '' AND `file` != '' ORDER BY `sort` ASC, `id` ASC";
            ...
            <div class="proinfo clearfix">
                <ul class="sidemenu">
                	<li class="sidelist<?=($ID == 0 ? " selected" : "")?>">
                		<a class="list_link" href="product.php">Hot Products</a>
<?php
    ...
    for ($i = 0; $i < sizeof($ProductClassList); $i++) {
        ...
        echo "                		<a class=\"list_link\" href=\"#\" onclick=\"return false;\">".$ProductClassList[$i]['name']."</a>\n";
        ...
        if (sizeof($ProductDataList) > 0) {
            ...
            for ($j = 0; $j < sizeof($ProductDataList); $j++)
                echo "                	     	<li><a href=\"product_info.php?id=".$ProductDataList[$j]['id']."\">".$ProductDataList[$j]['name']."</a></li>\n";
...
?>
                		    ...
                		    <p class="info_txt"><?=nl2br($ProductData['introduction'])?></p>
                		</div>
  <div id="tabs">
  <ul>
    <li><a href="#tabs-1">Expert Verification</a></li>
    <li><a href="#tabs-2">Features</a></li>
    <li><a href="#tabs-3">Specification</a></li>
    <li><a href="#tabs-4">Accessories</a></li>
    <li><a href="#tabs-5">File Link</a></li>
  </ul>
...
<?php
    ...
    if (sizeof($DownloadType1List) > 0) {
        ...
        echo "      <li>".$DownloadType[1]['en']."：";

        for ($i = 0; $i < sizeof($DownloadType1List); $i++)
            if ($DownloadType1List[$i]['file'] != "" && file_exists(Path_Upload.$DownloadType1List[$i]['file']))
                echo "<a href=\"".Url_Upload.$DownloadType1List[$i]['file']."\" target=\"_blank\">".$DownloadType1List[$i]['name']."</a>";
    ...
    if (sizeof($DownloadType2List) > 0) {
        ...
        echo "      <li>".$DownloadType[2]['en']."：";

        for ($i = 0; $i < sizeof($DownloadType2List); $i++)
            if ($DownloadType2List[$i]['file'] != "" && file_exists(Path_Upload.$DownloadType2List[$i]['file']))
                echo "<a href=\"".Url_Upload.$DownloadType2List[$i]['file']."\" target=\"_blank\">".$DownloadType2List[$i]['name']."</a>";
    ...
    if (sizeof($DownloadType3List) > 0) {
        ...
        echo "      <li>".$DownloadType[3]['en']."：";

        for ($i = 0; $i < sizeof($DownloadType3List); $i++)
            if ($DownloadType3List[$i]['file'] != "" && file_exists(Path_Upload.$DownloadType3List[$i]['file']))
                echo "<a href=\"".Url_Upload.$DownloadType3List[$i]['file']."\" target=\"_blank\">".$DownloadType3List[$i]['name']."</a>";
